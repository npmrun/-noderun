...
interface Its {name:string,type:string,childD:(Its|null)[],childF:IFile[]};
interface IFile {name:string,type:string};
 ...
 */
function needIgnore(name:string, ignoreList:RegExp[]) {
  var result = false;
 ...
 */
function dirDFS(path:string, dirName:string, ignoreList:RegExp[]):Its|any{
  // path是否合法
 ...
 */
function stringifyHelper(type:string, obj:Its, depth:number, isEnd:boolean[]) {
  var tmpStr = "";
  ...
  if (obj["type"] == "dir") {
    ...
    for (let i = 0; i < obj["childD"].length; i++) {
      ...
      tmpStr += stringifyHelper(type, <Its>obj["childD"][i], depth + 1, isEnd);
    }
    for (let i = 0; i < obj["childF"].length; i++) {
      isEnd[depth + 1] = --arrLength ? false : true;
      tmpStr += stringifyHelper(type, <Its>obj["childF"][i], depth + 1, isEnd);
    }
 ...
 */
function stringifyDirTree(dirTree:Its, type:string) {
  var str = "";
  var depth = 0;
  ...
  for (let i = 0; i < dirTree["childD"].length; i++) {
    isEnd[depth + 1] = --arrLength ? false : true;
    str += stringifyHelper(type, <Its>dirTree["childD"][i], depth + 1, isEnd);
  }
  for (let i = 0; i < dirTree["childF"].length; i++) {
    isEnd[depth + 1] = --arrLength ? false : true;
    str += stringifyHelper(type, <Its>dirTree["childF"][i], depth + 1, isEnd);
  }
